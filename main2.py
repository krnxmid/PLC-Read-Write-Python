...
def get_tags_data(client):
    ...
    for tag in GL_TAGS:
        ...
        # Send tag 5 as a String of characters by converting the value as ascii into characters
        if tag_name == "tag_5":
            char1, char2 = convert_16bit_to_ascii(low_word)
            char3, char4 = convert_16bit_to_ascii(high_word)
            final_char = char1+char2+char3+char4
            
            data.append([tag_name, final_char])
        else:
        ...
        try:
            ...
            if tag_name == "tag_5": # Recieve request for writing tag 5
                ...
                try:
                    ...
                    response2 = client.write_register(address=address, value = tag_higher)
                    response1 = client.write_register(address=address + 1, value = tag_lower)
